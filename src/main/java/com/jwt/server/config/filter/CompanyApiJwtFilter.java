...
public class CompanyApiJwtFilter extends GenericFilterBean {
    ...
    public void doFilter(ServletRequest servletRequest, ServletResponse servletResponse, FilterChain filterChain) {
        ...
        if (Objects.nonNull(token) && jwtHelper.validateAccessToken(token)) {
            ...
            if (AuthType.ACCOUNT.equals(claims.get(ACCOUNT_TYPE, AuthType.class))) {
            ...
            Authorization account =  this.companyRepository.findByLogin(login)
                    .orElseThrow(() -> new UsernameNotFoundException(COMPANY_NOT_FOUND));
